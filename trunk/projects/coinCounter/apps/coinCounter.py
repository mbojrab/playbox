...
def countCoins(image, batchSet, show=False, debug=False) :
    ...
    if circles is not None :
        for c in circles[0,:] :
            ...
            if debug :
                cv2.imwrite('chipNP_' + str(ii) + '.tiff', 
                            batchSet[chipPosition,0,:,:]*255.)

            # once the buffer is full classify it
            if (ii+1) % batchSet.shape[0] == 0 :
                results.extend(network.classify(batchSet))
            if debug :
        ...
        if remainder > 0:
            results.extend(network.classify(batchSet)[:remainder])

    totalValue = sum([coinValue[cl] for cl in results])

    if show :
        draw = ImageDraw.Draw(image)
        labels = [network.getNetworkLabels()[cl] for cl in results]
        ...
        if circles is not None :
            ...
            for c in circles[0,:] :
                ...
                radius, upperLeft, lowerRight = getChipArea(c)
                draw.rectangle([upperLeft, lowerRight], outline=(0,150,0))

                # put the label over the object
                draw.text([c[0]-(c[2]/1.2), c[1]], labels[ii],
