...
def sortDataset(netList, data, percentile=.95, debug=False) :
    ...
    imagery = data[0]
    labels = data[1]
    sims = pd.DataFrame(columns=['Similarity', 'Batch', 'Index',
                                 'Encoding', 'True Label'])
    ...
    for ii, batch in enumerate(imagery) :
        ...
        sims.append([ii // batchSize, ii % batchSize, cos / float(len(nets)),
                    enc, labels[ii]], ignore_index=True)
...
if __name__ == '__main__' :
    ...
    sortDataset(nets, test, options.percentile, options.debug)
